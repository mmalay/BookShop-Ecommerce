...
router.route('/:id').get((req, res) => {
  mysqlConnection.query(
    'SELECT shopping_cart.item_id, shopping_cart.quantity, product.name, product.price, product.image, product.product_id  FROM product INNER JOIN shopping_cart WHERE shopping_cart.product_id=product.product_id AND shopping_cart.customer_id=1;',
    (err, rows) => {
