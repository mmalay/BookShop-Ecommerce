...
router.route('/:id').get( (req, res) => {
    mysqlConnection.query(
      'SELECT orders.order_id, orders.created_on, orders.quantity, product.name, product.price, product.image  FROM product INNER JOIN orders WHERE orders.product_id=product.product_id AND orders.customer_id=1;',
      (err, rows) => {
